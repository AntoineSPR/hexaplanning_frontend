...
# Charge les utilitaires si présents
. "$(dirname "$0")/../../utils.sh"

# Vérification si le bypass est activé
if [ "$BYPASS_PROTECTED_FILES" = "true" ]; then
    echo_yellow "⚠️ Bypass active: Skipping protected files check."
    exit 0
fi

# Liste des fichiers protégés (compatibles avec sh et bash)
PROTECTED_FILES=".prettierrc.json eslint.config.js .husky/pre-commit"
...
# Vérification du dossier protégé
...
# Bloque le commit si nécessaire
